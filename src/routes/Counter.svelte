<script lang="ts">
  ...
  import { derived, $effect } from 'svelte/runes';
...
<div class="counter">
  <button on:click={() => (count.target -= 1)} aria-label="Decrease the counter by one">
  ...
  <button on:click={() => (count.target += 1)} aria-label="Increase the counter by one">
    <svg aria-hidden="true" viewBox="0 0 1 1">
